...
/* ----- 経路を取得する ----- */
vector<pair<string, string> > getPATH(vector<pair<string, string> > candidate,
                                      map<string, string> pages,
                                      string goalID) {
  vector<pair<string, string> > path;
  reverse(candidate.begin(),
          candidate.end());  // 逆順に探した方が効率がいいのでreverseする
...
/* ----- 幅優先探索(BFS) & 経路の表示 ----- */
void bfs(map<string, string> pages,
  ...
  if (ans) { /* --- 経路が見つかったとき --- */
    ...
    cout << "Shows the route from \"" << start << "\" to \"" << goal << "\"."
         << endl;
    for (pair<string, string> p : path) {
      cout << p.first << " (pageID : " << p.second << ")" << endl;
    }
  } else { /* --- 経路が見つからなかったとき --- */
    cout << "You can't go from \"" << start << "\" to \"" << goal << "\"."
         << endl;
  }
}
